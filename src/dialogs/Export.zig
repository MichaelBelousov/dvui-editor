...
pub fn dialog(id: dvui.Id) anyerror!bool {
    ...
    valid = inkz_editor.editor.activeFile() != null;
            ...
            if (anim.frames.len > 0) {
...

                {
...

                    {
                        ...
                        const alpha_triangles = inkz_editor.dvui.pathToSubdividedQuad(path.build(), dvui.currentWindow().arena(), .{
                            .subdivisions = 8,
    ...
    switch (response) {
        .ok => {
            switch (mode) {
                .animation => {
                    const default = blk: {
                        const file = inkz_editor.editor.activeFile() orelse {
                    ...
                    inkz_editor.backend.showSaveFileDialog(
...
pub fn createAnimationGif(path: []const u8) anyerror!void {
    ...
    var file = inkz_editor.editor.activeFile() orelse {
    ...
    if (file.selected_animation_index) |animation_index| {
        const anim: inkz_editor.Internal.Animation = file.animations.get(animation_index);
