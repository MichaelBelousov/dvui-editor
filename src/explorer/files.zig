    ...
    const recursor = struct {
        fn search(io: Io, directory: []const u8, tree: *inkz_editor.dvui.TreeWidget, inner_unique_id: dvui.Id, inner_id_extra: *usize, color_id: *usize, filter_text: []const u8, parent_branch: ?*inkz_editor.dvui.TreeWidget.Branch) !void {
            ...
            for (files.items) |entry| {
                ...
                if (inkz_editor.editor.explorer.open_branches.get(branch_id) != null) {
                    expanded = true;
                }
                ...
                if (branch.dropInto() and entry.kind == .directory) {
                    ...
                    inkz_editor.editor.explorer.open_branches.put(branch_id, {}) catch {};
                }
                    ...
                    if (context.activePoint()) |point| {
                        ...
                        if ((dvui.menuItemLabel(@src(), "New File...", .{}, .{ .expand = .horizontal })) != null) {
                            ...
                            var mutex = inkz_editor.dvui.dialog(@src(), .{
                                .displayFn = inkz_editor.Editor.Dialogs.NewFile.dialog,
                                .callafterFn = inkz_editor.Editor.Dialogs.NewFile.callAfter,
                                .title = "New File...",
                                .ok_label = "Create",
                                .cancel_label = "Cancel",
                                .resizeable = false,
                ...
                switch (entry.kind) {
                    ...
                    .directory => {
                        ...
                        if (branch.expander(@src(), .{ .indent = expanded_indent }, .{
                            ...
                            if (inkz_editor.editor.explorer.open_branches.contains(branch_id)) {
                                _ = inkz_editor.editor.explorer.open_branches.remove(branch_id);
                            }
                        }
