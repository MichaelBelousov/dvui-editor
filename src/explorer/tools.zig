...
pub fn draw(self: *Tools) !void {
    ...
    const layer_count: usize = if (inkz_editor.editor.activeFile()) |file| file.layers.len else 0;
    defer prev_layer_count = layer_count;

    var paned = inkz_editor.dvui.paned(@src(), .{
        .direction = .vertical,
        .collapsed_size = 0,
        .handle_size = 10,
        .handle_dynamic = .{},
    ...
    if (paned.dragging) {
        ...
        inkz_editor.editor.explorer.layers_ratio = paned.split_ratio.*;
    }
    ...
    if (((dvui.firstFrame(paned.data().id) or prev_layer_count != layer_count) or autofit) and !inkz_editor.editor.explorer.pinned_palettes) {
        ...
        if (dvui.firstFrame(paned.data().id)) {
            if (layer_count == 0)
                ...
                paned.split_ratio.* = inkz_editor.editor.explorer.layers_ratio;
...
pub fn drawLayers(tools: *Tools) !?dvui.Rect.Physical {
    ...
    if (inkz_editor.editor.activeFile()) |file| {
        ...
        var tree = inkz_editor.dvui.TreeWidget.tree(@src(), .{ .enable_reordering = true }, .{
            .expand = .horizontal,
            .background = false,
        });
        defer tree.deinit();
        ...
        if (insert_before_index) |insert_before| {
            if (removed_index) |removed| {
                const prev_order = try inkz_editor.app.allocator.alloc(u64, file.layers.len);
                ...
                if (insert_before <= file.layers.len) {
                    file.layers.insert(inkz_editor.app.allocator, if (removed < insert_before) insert_before - 1 else insert_before, layer) catch {
                        dvui.log.err("Failed to insert layer", .{});
                    };
                } else {
                    file.layers.insert(inkz_editor.app.allocator, if (removed < insert_before) file.layers.len else 0, layer) catch {
                        dvui.log.err("Failed to insert layer", .{});
                    };
                }
                ...
                if (!std.mem.eql(u64, file.layers.items(.id)[0..file.layers.len], prev_order)) {
                    ...
                    inkz_editor.app.allocator.free(prev_order);
                }
                ...
                if (should_commit_rename) {
                    if (!std.mem.eql(u8, file.layers.items(.name)[layer_index], te.getText()) and te.getText().len > 0) {
                        file.history.append(.{
                            .layer_name = .{
                                .index = layer_index,
                                .name = try inkz_editor.app.allocator.dupe(u8, file.layers.items(.name)[layer_index]),
                            },
                        }) catch {
                            dvui.log.err("Failed to append history", .{});
                        };
                        inkz_editor.app.allocator.free(file.layers.items(.name)[layer_index]);
                        file.layers.items(.name)[layer_index] = try inkz_editor.app.allocator.dupe(u8, te.getText());
                    }
                    if (te.enter_pressed) {
        ...
        if (vertical_scroll > 0.0)
            inkz_editor.dvui.drawEdgeShadow(scroll_area.data().contentRectScale(), .top, .{});
        ...
        if (file.editor.layers_scroll_info.virtual_size.h > file.editor.layers_scroll_info.viewport.h and vertical_scroll < file.editor.layers_scroll_info.scrollMax(.vertical))
            inkz_editor.dvui.drawEdgeShadow(scroll_area.data().contentRectScale(), .bottom, .{});
    }

    if (inkz_editor.dvui.hovered(vbox.data())) {
        const mp = dvui.currentWindow().mouse_pt;
...
pub fn drawColors() !void {
    ...
    const primary: dvui.Color = .{ .r = inkz_editor.editor.colors.primary[0], .g = inkz_editor.editor.colors.primary[1], .b = inkz_editor.editor.colors.primary[2], .a = inkz_editor.editor.colors.primary[3] };
    const secondary: dvui.Color = .{ .r = inkz_editor.editor.colors.secondary[0], .g = inkz_editor.editor.colors.secondary[1], .b = inkz_editor.editor.colors.secondary[2], .a = inkz_editor.editor.colors.secondary[3] };
    ...
    var clicked: bool = false;
    {
        ...
        drawColorPicker(primary_button.data().rectScale().r, &inkz_editor.editor.colors.primary) catch {};
...

    {
        ...
        drawColorPicker(secondary_button.data().rectScale().r, &inkz_editor.editor.colors.secondary) catch {};

        if (secondary_button.clicked()) clicked = true;
    }

    if (clicked) {
        std.mem.swap([4]u8, &inkz_editor.editor.colors.primary, &inkz_editor.editor.colors.secondary);
    }
}
...
pub fn drawPaletteControls() !void {
    ...
    if (dvui.buttonIcon(@src(), "PinPalettes", dvui.entypo.pin, .{ .draw_focus = false }, .{}, .{
        ...
        .style = if (inkz_editor.editor.explorer.pinned_palettes) .highlight else .control,
    })) {
        inkz_editor.editor.explorer.pinned_palettes = !inkz_editor.editor.explorer.pinned_palettes;
    }
}
                    ...
                    .file => |data| {
                        ...
                        if (std.mem.eql(u8, ext, ".hex")) {
                            if (dropdown.addChoiceLabel(entry.name)) {
                                inkz_editor.editor.colors.palette = inkz_editor.Internal.Palette.loadFromBytes(inkz_editor.app.allocator, entry.name, data) catch |err| {
...

    {
        if (inkz_editor.editor.colors.palette) |*palette| {
            var flex_box = dvui.flexbox(@src(), .{ .justify_content = .start }, .{
                .expand = .horizontal,
                .max_size_content = .{
                    .w = inkz_editor.editor.explorer.rect.w - 20 * dvui.currentWindow().natural_scale,
                    .h = inkz_editor.editor.explorer.rect.h - 20 * dvui.currentWindow().natural_scale,
                },
            });
                    ...
                    switch (evt) {
                        .mouse => |mouse_evt| {
                            switch (mouse_evt.button) {
                                .left => {
                                    @memcpy(&inkz_editor.editor.colors.primary, &color);
                                },
                                .right => {
                                    @memcpy(&inkz_editor.editor.colors.secondary, &color);
                                },
...
fn searchPalettes(dropdown: *dvui.DropdownWidget) !void {
    var dir_opt = std.fs.cwd().openDir(inkz_editor.editor.palette_folder, .{ .access_sub_paths = false, .iterate = true }) catch null;
    if (dir_opt) |*dir| {
        defer dir.close();
            ...
            if (entry.kind == .file) {
                ...
                if (std.mem.eql(u8, ext, ".hex")) {
                    ...
                    if (dropdown.addChoiceLabel(label)) {
                        const abs_path = try std.fs.path.join(dvui.currentWindow().arena(), &.{ inkz_editor.editor.palette_folder, entry.name });

                        if (inkz_editor.editor.colors.palette) |*palette|
                            palette.deinit();

                        inkz_editor.editor.colors.palette = inkz_editor.Internal.Palette.loadFromFile(inkz_editor.app.allocator, abs_path) catch |err| {
                            dvui.log.err("Failed to load palette: {s}", .{@errorName(err)});
                            return error.FailedToLoadPalette;
                        };
                    }
...
fn layerGestureMatches(file: *const inkz_editor.Internal.File) bool {
...
fn layerTreeClearGestureKeysOnly(_: *const inkz_editor.Internal.File) void {
    layer_row_gesture = null;
}

/// Clear gesture and global `Dragging` (stale prestart/drag from other widgets).
fn layerTreeResetRowPointerGesture(_: *const inkz_editor.Internal.File) void {
    dvui.dragEnd();
    layer_row_gesture = null;
}
...
fn layerTreePointerInTreeSurface(tree: *inkz_editor.dvui.TreeWidget, p: dvui.Point.Physical, floating_win: dvui.Id) bool {
...
fn layerTreePointerInTreeBorder(tree: *inkz_editor.dvui.TreeWidget, p: dvui.Point.Physical, floating_win: dvui.Id) bool {
    if (floating_win != dvui.subwindowCurrentId()) return false;
...
fn layerTreeMotionAllowsLayerReorder(tree: *inkz_editor.dvui.TreeWidget, e: *dvui.Event) bool {
...
fn processLayerTreePointerEvents(tree: *inkz_editor.dvui.TreeWidget, file: *inkz_editor.Internal.File, hits: []const LayerRowHit, layers_viewport_r: ?dvui.Rect.Physical) void {
