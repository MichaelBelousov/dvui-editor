...
pub fn drawAnimations(self: *Sprites) !void {
    ...
    if (inkz_editor.editor.activeFile()) |file| {
        ...
        defer {
            if (file.editor.animations_scroll_info.viewport.w < file.editor.animations_scroll_info.virtual_size.w) {
                if (file.editor.animations_scroll_info.offset(.horizontal) < file.editor.animations_scroll_info.scrollMax(.horizontal)) {
                    inkz_editor.dvui.drawEdgeShadow(scroll_area.data().contentRectScale(), .right, .{});
                }
                if (file.editor.animations_scroll_info.offset(.horizontal) > 0.0) {
                    inkz_editor.dvui.drawEdgeShadow(scroll_area.data().contentRectScale(), .left, .{});
                }
            }
        }
        ...
        var reorderable = inkz_editor.dvui.reorder(@src(), .{ .drag_name = "anim_drag" }, .{
            .expand = .horizontal,
            .background = false,
        });
        ...
        if (self.animation_insert_before_index) |insert_before| {
            if (self.animation_removed_index) |removed| {
                //const prev_order = try inkz_editor.app.allocator.dupe(inkz_editor.Animation.Frame, file.animations.items(.id));

                const anim = file.animations.get(removed);
                file.animations.orderedRemove(removed);

                if (insert_before <= file.animations.len) {
                    file.animations.insert(inkz_editor.app.allocator, if (removed < insert_before) insert_before - 1 else insert_before, anim) catch {
                        dvui.log.err("Failed to insert animation", .{});
                    };
                } else {
                    file.animations.insert(inkz_editor.app.allocator, if (removed < insert_before) file.animations.len else 0, anim) catch {
                        dvui.log.err("Failed to insert animation", .{});
                    };
                }
        ...
        for (file.animations.items(.id), 0..) |anim_id, anim_index| {
            ...
            const hovered = inkz_editor.dvui.hovered(r.data());
            ...
            _ = inkz_editor.dvui.ReorderWidget.draggable(@src(), .{
                .reorderable = r,
                ...
                if (te.enter_pressed or dvui.focusedWidgetId() != te.data().id) {
                    if (!std.mem.eql(u8, file.animations.items(.name)[anim_index], te.getText()) and te.getText().len > 0) {
                        file.history.append(.{
                            .animation_name = .{
                                .index = anim_index,
                                .name = try inkz_editor.app.allocator.dupe(u8, file.animations.items(.name)[anim_index]),
                            },
                        }) catch {
                            dvui.log.err("Failed to append history", .{});
                        };
                        inkz_editor.app.allocator.free(file.animations.items(.name)[anim_index]);
                        file.animations.items(.name)[anim_index] = try inkz_editor.app.allocator.dupe(u8, te.getText());
                    }
    ...
    if (inkz_editor.editor.activeFile()) |file| {
...

        {
            ...
            if (sort_anim_asc_button.clicked()) {
                const prev_order = try inkz_editor.app.allocator.dupe(inkz_editor.Animation.Frame, animation.frames);
                std.mem.sort(inkz_editor.Animation.Frame, animation.frames, {}, FrameSort.asc);
        ...
        {
            {
                ...
                if (sort_anim_desc_button.clicked()) {
                    const prev_order = try inkz_editor.app.allocator.dupe(inkz_editor.Animation.Frame, animation.frames);
                    std.mem.sort(inkz_editor.Animation.Frame, animation.frames, {}, FrameSort.desc);
...

        {
            ...
            if (add_sprite_button.clicked()) {
                if (file.editor.selected_sprites.count() > 0) {
                    ...
                    var frames = std.array_list.Managed(inkz_editor.Animation.Frame).init(dvui.currentWindow().arena());
                    ...
                    animation.appendFrames(inkz_editor.app.allocator, frames.items) catch {
...

        {
            ...
            if (duplicate_animation_button.clicked()) {
                ...
                while (iter.next()) |sprite_index| {
                    for (animation.frames) |frame| {
                        if (frame.sprite_index == sprite_index) {
                            try animation.appendFrame(inkz_editor.app.allocator, .{
                ...
                while (iter.next()) |sprite_index| {
                    ...
                    while (i > 0) : (i -= 1) {
                        if (animation.frames[i - 1].sprite_index == sprite_index) {
                            animation.removeFrame(inkz_editor.app.allocator, i - 1);
                            break;
                        }
                    }
                }
...
pub fn drawFrames(self: *Sprites) !void {
    if (inkz_editor.editor.activeFile()) |file| {
        ...
        if (file.selected_animation_index) |animation_index| {
            ...
            var reorder = inkz_editor.dvui.reorder(@src(), .{ .drag_name = "sprite_drag" }, .{
                .expand = .horizontal,
                .background = false,
            });
            ...
            if (self.sprite_insert_before_index) |insert_before| {
                if (self.sprite_removed_index) |removed| {
                    ...
                    dvui.ReorderWidget.reorderSlice(inkz_editor.Animation.Frame, animation.frames, removed, insert_before);
...
const FrameSort = struct {
    pub fn asc(_: void, a: inkz_editor.Animation.Frame, b: inkz_editor.Animation.Frame) bool {
        return a.sprite_index < b.sprite_index;
    }

    pub fn desc(_: void, a: inkz_editor.Animation.Frame, b: inkz_editor.Animation.Frame) bool {
        return a.sprite_index > b.sprite_index;
    }
};
