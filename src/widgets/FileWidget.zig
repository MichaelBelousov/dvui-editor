...
fn sample(self: *FileWidget, file: *inkz_editor.Internal.File, point: dvui.Point, change_layer: bool, change_tool: bool) void {
    ...
    if (file.editor.isolate_layer) {
        ...
        } else if (!inkz_editor.editor.explorer.tools.layersHovered()) {
    ...
    if (change_tool) {
        if (color[3] == 0) {
            ...
            if (switch (inkz_editor.editor.tools.current) {
                .pencil, .bucket => false,
                else => true,
            })
...
pub fn processAnimationSelection(self: *FileWidget) void {
    ...
    for (dvui.events()) |*e| {
        ...
        switch (e.evt) {
            .mouse => |me| {
                if ((me.button.pointer() and me.action == .press and !me.mod.matchBind("ctrl/cmd") and !me.mod.matchBind("shift")) or (inkz_editor.editor.tools.current != .pointer and self.sample_data_point == null)) {
...
pub fn processCellReorder(self: *FileWidget) void {
    if (inkz_editor.editor.tools.current != .pointer) return;
                            ...
                            if (di != file.spriteIndex(current_point)) {
                                ...
                                if (self.removed_sprite_indices) |removed_sprite_indices| {
                                    if (self.insert_before_sprite_indices) |insert_before_sprite_indices| {
                                        inkz_editor.app.allocator.free(insert_before_sprite_indices);
                                        self.insert_before_sprite_indices = null;
                                    }

                                    // This will actually trigger the drag/drop
                                    var insert_before_sprite_indices = inkz_editor.app.allocator.alloc(usize, file.editor.selected_sprites.count()) catch {
                                    ...
                                    file.history.append(.{
                                        .reorder_cell = .{
                                            .removed_sprite_indices = inkz_editor.app.allocator.dupe(usize, removed_sprite_indices) catch {
                                                dvui.log.err("Failed to duplicate removed sprite indices", .{});
                                                return;
                                            },
                                            .insert_before_sprite_indices = inkz_editor.app.allocator.dupe(usize, insert_before_sprite_indices) catch {
                    ...
                    if (dvui.captured(file.editor.canvas.scroll_container.data().id)) {
                        if (dvui.dragging(me.p, "sprite_reorder_drag")) |_| {
                            ...
                            if (self.removed_sprite_indices == null and file.editor.selected_sprites.count() > 0) {
                                var removed_sprite_indices = inkz_editor.app.allocator.alloc(usize, file.editor.selected_sprites.count()) catch {
...
fn bubblePanSharedForGrid(self: *FileWidget) ?BubblePanShared {
    ...
    const tool_not_pointer = inkz_editor.editor.tools.current != .pointer;
...
pub fn drawSpriteBubbles(self: *FileWidget) void {
    ...
    const mod_shift = cw.modifiers.matchBind("shift");
    const mod_ctrl_cmd = cw.modifiers.matchBind("ctrl/cmd");
    const radial_visible = inkz_editor.editor.tools.radial_menu.visible;
    const sample_active = self.sample_data_point != null;
    ...
    if (bubble_rect_scale.r.h <= dvui.currentWindow().natural_scale) {
        ...
        if (button.clicked()) { // Toggle animation frame on or off for this selection/animation
            if (self.init_options.file.selected_animation_index) |anim_index| {
                ...
                var frames = std.array_list.Managed(inkz_editor.Animation.Frame).init(inkz_editor.app.allocator);
                ...
                if (!anim.eqlFrames(frames.items)) {
                    self.init_options.file.history.append(.{
                        .animation_frames = .{
                            .index = anim_index,
                            .frames = inkz_editor.app.allocator.dupe(inkz_editor.Animation.Frame, anim.frames) catch {
                    ...
                    inkz_editor.app.allocator.free(anim.frames);
                ...
                if (self.init_options.file.createAnimation() catch null) |anim_index| {
                    ...
                    inkz_editor.editor.explorer.sprites.edit_anim_id = self.init_options.file.animations.items(.id)[anim_index];
                    inkz_editor.editor.explorer.pane = .sprites;

                    var anim = self.init_options.file.animations.get(anim_index);
                    if (anim.frames.len == 0) {
                        anim.appendFrame(inkz_editor.app.allocator, .{ .sprite_index = sprite_index, .ms = temp_ms }) catch {
                            dvui.log.err("Failed to append frame", .{});
                            return false;
                        };
                    }
                ...
                if (update) {
                    const current_point = self.init_options.file.editor.canvas.dataFromScreenPoint(dvui.currentWindow().mouse_pt);
                    {
                        ...
                        file.drawPoint(
                            ...
                            .{
                                .mask_only = true,
                                .stroke_size = inkz_editor.editor.tools.stroke_size,
                            },
                        );
...
pub fn processStroke(self: *FileWidget) void {
    const file = self.init_options.file;
    const stroke_size = inkz_editor.editor.tools.stroke_size;
    ...
    const color: [4]u8 = switch (inkz_editor.editor.tools.current) {
        .pencil => inkz_editor.editor.colors.primary,
        .eraser => [_]u8{ 0, 0, 0, 0 },
        else => unreachable,
    };
                            ...
                            if (me.mod.matchBind("shift")) {
                                ...
                                if (self.drag_data_point) |previous_point| {
                                    if (file.strokeUndoExpandToCoverRect(file.lineBrushCoverRect(previous_point, current_point, stroke_size))) |_| {
                                        ...
                                        inkz_editor.perf.draw_event_count += 1;
                                    } else |err| {
                                        dvui.log.err("strokeUndoExpandToCoverRect failed: {}", .{err});
                                    }
                                }
                                ...
                                if (self.init_options.file.editor.canvas.rect.contains(me.p) and self.sample_data_point == null) {
                                    if (self.sample_data_point == null or color[3] == 0) {
                                        clearTempPreview(&file.editor);
                                        const temp_color = if (inkz_editor.editor.tools.current != .eraser) color else [_]u8{ 255, 255, 255, 255 };
                                        file.drawPoint(
                                            current_point,
                                            .temporary,
                                            .{
...
pub fn processFill(self: *FileWidget) void {
    if (inkz_editor.editor.tools.current != .bucket) return;
    if (self.sample_key_down) return;
    const file = self.init_options.file;
    const color = inkz_editor.editor.colors.primary;
    ...
    if (file.editor.transform) |*transform| {
        ...
        if (triangle_opts) |*triangles| {
            ...
            for (transform.data_points[0..6], 0..) |*data_point, point_index| {
                const transform_point = @as(inkz_editor.Editor.Transform.TransformPoint, @enumFromInt(point_index));
                ...
                for (dvui.events()) |*e| {
                    ...
                    if (screen_rect.contains(dvui.currentWindow().mouse_pt)) {
                        ...
                        if (active_point == @as(inkz_editor.Editor.Transform.TransformPoint, @enumFromInt(point_index))) {
                            dvui.cursorSet(.hand);
                        }
                    }
                                        ...
                                        if (transform.active_point) |active_point| {
                                            if (@intFromEnum(active_point) == point_index) {
                                                ...
                                                if (point_index < 4) {
                                                    ...
                                                    new_point = inkz_editor.math.rotate(new_point, transform.point(.pivot).*, -transform.rotation);
                                                    ...
                                                    if (me.mod.matchBind("ctrl/cmd")) {
                                                        ...
                                                        blk_vert: {
                                                            ...
                                                            var rotation_direction: dvui.Point = inkz_editor.math.rotate(dvui.Point{ .x = 1, .y = 0 }, transform.point(.pivot).*, 0);
                                                            var rotation_perp: dvui.Point = inkz_editor.math.rotate(dvui.Point{ .x = 0, .y = 1 }, transform.point(.pivot).*, 0);
                                                ...
                                                if (transform_point == .rotate) {
                                                    if (self.drag_data_point) |drag_data_point| {
                                                        ...
                                                        if (me.mod.matchBind("ctrl/cmd")) { // Lock rotation to cardinal directions
                                                            const direction = inkz_editor.math.Direction.fromRadians(transform.rotation);
...
pub fn drawTransform(self: *FileWidget) void {
    ...
    if (file.editor.transform) |*transform| {
        ...
        centroid = inkz_editor.math.rotate(centroid, transform.point(.pivot).*, transform.rotation);
                ...
                if (show_ortho_dims) {
                    ...
                    if (dim_cell_opt) |ci| {
                        ...
                        // Transform width (bottom edge) and height (left edge): labels only, no dimension lines.
                        {
                            ...
                            const offset_v = inkz_editor.math.rotate(
                            ...
                            const offset_h = inkz_editor.math.rotate(
...
fn fileCanvasVisibleGridParams(file: *inkz_editor.Internal.File) ?struct {
...
fn spriteAnimationPaletteColor(file: *inkz_editor.Internal.File, sprite_index: usize) ?dvui.Color {
    if (inkz_editor.editor.colors.file_tree_palette) |*palette| {
        var animation_index: ?usize = null;
...
fn checkerboardCellCornerColor(
    effect: inkz_editor.Editor.Settings.TransparencyEffect,
    file: *inkz_editor.Internal.File,
    sprite_index: usize,
    c_tl: dvui.Color,
    c_tr: dvui.Color,
...
fn checkerboardTintAtSpriteCellCenter(file: *inkz_editor.Internal.File, sprite_index: usize) dvui.Color {
    const pal = checkerboardGridPalette();
    const tone = pal.tone;
    switch (inkz_editor.editor.settings.transparency_effect) {
        .none => return tone,
        .rainbow => {
...
fn drawCheckerboardCellsBatched(file: *inkz_editor.Internal.File) void {
    const n = file.spriteCount();
    if (n == 0) return;

    const te = inkz_editor.editor.settings.transparency_effect;
    const pal = checkerboardGridPalette();
    const tone = pal.tone;
...
pub fn drawCursor(self: *FileWidget) void {
    if (inkz_editor.editor.tools.current == .pointer and self.sample_data_point == null) return;
    if (inkz_editor.editor.tools.radial_menu.visible) return;
    if (self.init_options.file.editor.transform != null) return;
    ...
    if (switch (inkz_editor.editor.tools.current) {
        ...
        .selection => if (subtract) inkz_editor.editor.atlas.data.sprites[inkz_editor.atlas.sprites.selection_rem_default] else if (add) inkz_editor.editor.atlas.data.sprites[inkz_editor.atlas.sprites.selection_add_default] else inkz_editor.editor.atlas.data.sprites[inkz_editor.atlas.sprites.selection_default],
        else => null,
    }) |sprite| {
        const atlas_size = dvui.imageSize(inkz_editor.editor.atlas.source) catch {
...
pub fn drawSample(self: *FileWidget) void {
    ...
    if (point) |data_point| {
        ...
        inkz_editor.render.renderLayers(.{
            .file = file,
            .rs = rs,
            .uv = uv_rect,
            .corner_radius = .{
...
pub fn drawLayers(self: *FileWidget) void {
    const perf_t0 = inkz_editor.perf.drawLayersBegin();
    defer inkz_editor.perf.drawLayersEnd(perf_t0);
    ...
    if (inkz_editor.editor.tools.current == .pointer and file.editor.transform == null and self.resize_data_point == null) {
        var iter = file.editor.selected_sprites.iterator(.{ .kind = .set, .direction = .forward });
        while (iter.next()) |i| {
            ...
            if (inkz_editor.editor.explorer.pane == .sprites) {
...
fn drawReorderPreviewForAxis(
    self: *FileWidget,
    workspace: *inkz_editor.Editor.Workspace,
    file: *inkz_editor.Internal.File,
    axis: ReorderAxis,
    target_index: ?usize,
    removed_index: usize,
    ...
    defer {
        ...
        {
            inkz_editor.dvui.drawEdgeShadow(.{ .r = file.editor.canvas.screenFromDataRect(animated_target_box_rect), .s = scale }, if (axis == .columns) .right else .top, .{
                .opacity = 0.5,
            });
            inkz_editor.dvui.drawEdgeShadow(.{ .r = file.editor.canvas.screenFromDataRect(animated_target_box_rect), .s = scale }, if (axis == .columns) .left else .bottom, .{
                .opacity = 0.5,
            });
        }
                ...
                while (iter.next()) |sprite_index| {
                    ...
                    if (left_index) |left_index_value| {
                        if (!temp_selected_sprite.isSet(left_index_value)) {
                            inkz_editor.dvui.drawEdgeShadow(image_rect_scale, .left, .{ .opacity = 0.35 });
                        }
                    }
                    if (right_index) |right_index_value| {
                        if (!temp_selected_sprite.isSet(right_index_value)) {
                            inkz_editor.dvui.drawEdgeShadow(image_rect_scale, .right, .{ .opacity = 0.35 });
                        }
                    }
                    if (top_index) |top_index_value| {
                        if (!temp_selected_sprite.isSet(top_index_value)) {
                            inkz_editor.dvui.drawEdgeShadow(image_rect_scale, .top, .{ .opacity = 0.35 });
                        }
                    }
                    if (bottom_index) |bottom_index_value| {
                        if (!temp_selected_sprite.isSet(bottom_index_value)) {
                            inkz_editor.dvui.drawEdgeShadow(image_rect_scale, .bottom, .{ .opacity = 0.35 });
                        }
                    }
                }
            }
...
pub fn processEvents(self: *FileWidget) void {
    ...
    if (self.hovered() and self.active()) {
        const pe_t0 = inkz_editor.perf.processEventsBegin();
        defer inkz_editor.perf.processEventsEnd(pe_t0);

        const editor = &self.init_options.file.editor;
        if (editor.temp_preview_dirty_rect) |dirty| {
            if (dirty.w > 0 and dirty.h > 0) {
                inkz_editor.image.clearRect(editor.temporary_layer.source, dirty);
        ...
        {
            const mask_t0 = inkz_editor.perf.updateMaskBegin();
            defer inkz_editor.perf.updateMaskEnd(mask_t0);
            self.updateActiveLayerMask();
        }

        if (inkz_editor.editor.tools.current == .selection) {
        ...
        if (self.init_options.file.editor.transform == null) {
            const tool_t0 = inkz_editor.perf.toolProcessBegin();
            switch (inkz_editor.editor.tools.current) {
            ...
            self.processSample();
            inkz_editor.perf.toolProcessEnd(tool_t0);
        }
    }
    ...
    inkz_editor.dvui.drawEdgeShadow(self.init_options.file.editor.canvas.scroll_container.data().rectScale(), .top, .{ .opacity = 0.15 });
    inkz_editor.dvui.drawEdgeShadow(self.init_options.file.editor.canvas.scroll_container.data().rectScale(), .bottom, .{});
    inkz_editor.dvui.drawEdgeShadow(self.init_options.file.editor.canvas.scroll_container.data().rectScale(), .left, .{ .opacity = 0.15 });
    inkz_editor.dvui.drawEdgeShadow(self.init_options.file.editor.canvas.scroll_container.data().rectScale(), .right, .{});
...
fn tempStrokePreviewClipRect(canvas: *CanvasWidget, file: *const inkz_editor.Internal.File, stroke_size: usize) dvui.Rect {
...
fn expandTempGpuDirtyRect(editor: *inkz_editor.Internal.File.EditorData, rect: dvui.Rect) void {
...
/// new position.
fn clearTempPreview(editor: *inkz_editor.Internal.File.EditorData) void {
    if (editor.temp_preview_dirty_rect) |dirty| {
        if (dirty.w > 0 and dirty.h > 0) {
            inkz_editor.image.clearRect(editor.temporary_layer.source, dirty);
            expandTempGpuDirtyRect(editor, dirty);
